...
const safeRules = (s) => s.rules_html || esc(s.rules).replaceAll('\n', '<br>');
...
function shell(s, sponsors, body, active = '') {
  ...
  const sponsorHtml = sponsors.length ? `<footer><div class="wrap"><div class="sponsor-title">Unterstützt von</div><div class="sponsors">${sponsors.map((x) => {
    ...
    return x.url ? `<a href="${esc(x.url)}" rel="noopener noreferrer" target="_blank">${inner}</a>` : `<div>${inner}</div>`;
  }).join('')}</div></div></footer>` : '';
...
app.get('/turnier.pdf', async (q, r) => {
  try { r.type('pdf').set('Content-Disposition', 'attachment; filename="turnier.pdf"').send(await tournamentPdf()); }
  catch (e) { if (e.message === 'tournament_not_finished') return r.status(404).send('Der Turnierbericht steht erst nach Festlegung des Siegers bereit.'); throw e; }
});
...
app.get('/', async (q, r) => {
  ...
  const winner = s.winner_team_id ? (await db.query('SELECT name FROM teams WHERE id=$1', [s.winner_team_id])).rows[0] : null;
  s.winner_name = winner?.name || '';
